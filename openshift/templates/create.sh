...
if [[ -z "${ENV_NAME}" || -z "${API_BASE}" || -z "${KEYCLOAK_REALM}" || -z "${KEYCLOAK_URL}" || -z "${JWT_ISSUER_URI}" ]]
then
...
#oc process -f ./configmap.yaml  KEYCLOAK_REALM=${KEYCLOAK_REALM} KEYCLOAK_URL=${KEYCLOAK_URL} JWT_ISSUER_URI=${JWT_ISSUER_URI} ENV_NAME=${ENV_NAME} | oc apply -f -

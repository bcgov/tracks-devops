...
if [[ -z "${ENV_NAME}" || -z "${JWT_ISSUER_URI}" || -z "${KEYCLOAK_URL}" || -z "${KEYCLOAK_REALM}" || -z "${API_BASE}" ]]
then
...
oc process -f ./tracks-configmap.yaml ENV_NAME=${ENV_NAME} JWT_ISSUER_URI=${JWT_ISSUER_URI} KEYCLOAK_REALM=${KEYCLOAK_REALM} KEYCLOAK_URL=${KEYCLOAK_URL} BACKEND_SPRING_PROFILES="${BACKEND_SPRING_PROFILES}" | oc apply -f -
oc process -f ./database-secrets.yaml  DATABASE_PASSWORD=${DATABASE_PASSWORD} DATABASE_REPLICATION_PASSWORD=${DATABASE_REPLICATION_PASSWORD} DATABASE_ROOT_PASSWORD=${DATABASE_ROOT_PASSWORD}  | oc
apply -f -
oc process -f ./postgres-crunchy.yaml CPU_REQUEST=500m CPU_LIMIT=1000m MEMORY_REQUEST=1000M MEMORY_LIMIT=3G ENV_NAME=${ENV_NAME} | oc apply -f -
